...
export function QuestionForm({ roomId }: { roomId: string }) {
  ...
  const { mutateAsync: createQuestion } = useCreateQuestion(roomId, form.reset)

  const handleCreateQuestion = async ({ question }: CreateQuestionFormData) => {
        ...
        <Form {...form}>
          ...
          >
            ...
            <Button
              ...
              type="submit"
            >
              {form.formState.isSubmitting ? 'Enviando...' : 'Enviar pergunta'}
            </Button>
          </form>
        </Form>
      </CardContent>
